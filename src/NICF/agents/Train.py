...
class Train(object):
    ...
    def collecting_data_update_model(self, type="training", epoch=0):
        ...
        for item in self.args.ST:
            logger.record_tabular(str(item)+"precision",round(pd.Series([i["precision"] for i in infos[item]]).mean(),4))
            logger.record_tabular(str(item)+"recall",round(pd.Series([i["recall"] for i in infos[item]]).mean(),4))
            logger.log(str(item)+" precision: ",round(pd.Series([i["precision"] for i in infos[item]]).mean(),4))
